...
| `/new` | 结束当前绑定并**创建接入一个全新会话**：旧流最后一张卡灰化定稿 + 🆕 绿色边界卡分隔（聊天历史不删除）；新会话**继承旧会话的 cwd 和模型路由**（provider/model——裸建的 agent 没有路由，第一个请求会报 no provider/model）；`/resume` 可回旧会话；创建失败回复具体原因 |
