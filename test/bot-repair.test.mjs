...
/** Fake repair backend: records every call. */
function fakeRepair(overrides = {}) {
  ...
  return {
    ...
    async runRepair(logPath, options) {
      calls.run.push({ logPath, options })
 ...
 * session, the binder locates it in a REAL tmp dir, and the first
 * `corruptFails` binds throw the corrupt error
...
test('picking repair swaps the rebuilt log in and re-enters the session', async () => {
  const repair = fakeRepair()
  ...
  try {
    ...
    // Rebuild ran, swap followed, verify saw the candidate.
    ...
    // The phone saw the success + entered announcements.
...
test('a racer taking the session before the re-entry degrades to the read-only watch', async () => {
  const repair = fakeRepair()
  const { bot, sends, bindCalls, watchCalls, cleanup } = repairBot({
    repair,
    // The post-repair bind loses the race: another process owns the session
    // under the host's write lease (the exact refusal the host raises).
    bindImpl() {
      throw new SessionAlreadyOwnedError('torn-log')
    },
  })
  try {
    await bot.resumePickCore(1)
    ...
    assert.ok(texts.some(t => t.includes('另一个进程')), JSON.stringify(texts))
