...
// removed on success.
//
// dsh-feishu note: the scratch profile configures no Lark credentials and no
// operators, so the bot resolves to its documented dormant path — the smoke
// proves the plugin LOADS and APPLIES under the real host, which is exactly
...
const ownName = pkg.name // @aiwayds/dsh-feishu

const work = mkdtempSync(path.join(tmpdir(), 'dsh-feishu-smoke-'))
...
if (dumpAfter.stdout.includes('dsh-feishu')) {
